...
**Option A (Recommended):** Apply the defensive `expand_range()` fix now (1 hour, zero risk)

**Option B:** Do nothing — document as known latent issue, revisit when trainer goes postflop

**Option C:** Deprecate V1/V3 now and route all FE traffic through V2/V4 (higher effort but eliminates divergence permanently)
...
1. **Keep the defensive fix** — add `expand_range()` to V1/V3 as future-proofing (1 hour, zero risk)
