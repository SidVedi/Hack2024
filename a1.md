...
**Status:** INVESTIGATION COMPLETE — Bug is latent, not actively triggering for any tested scenario.
**Effort spent:** ~0.5 day (investigation only, no code changes made)

---

## Summary

V1 (`GenerateNextHand`) and V2 (`GenerateNextHandV2`) use different code paths to produce the `range_array` passed to `_generate_hand_response()`. After extensive hands-on debugging with breakpoints across **7 test scenarios** covering both game variants (Holdem + Omaha), all spot types, multiple sites and blind structures, and both cash and spins formats — **no active divergence was found**. The bug is latent. It would only activate if preflop tree nodes begin using sparse index compression (`indices` field), which none currently do.
...
**Conclusion:** Strategy and EV arrays are decoded from the same tree node bytes via `decode_strategy_ev()` in both V1 and V2. For preflop, both paths produce identical results. Tests 1-3 (`full_tree` and `postflop_only`) produce byte-for-byte identical strategy and EV arrays at the preflop level. Test 4 shows different data because it hits deeper nodes in a different tree (GGPoker Cash Drop preflop_only). Test 6 is the first with 4 actions and mixed strategy probabilities. In all cases the decoding path is the same, and V1 reads the stored data correctly.
...
Across 7 tests covering the full parameter space that the V1 trainer endpoint serves:
...
**`'indices' in tree_node` was `False` in every single test.** No preflop tree node in the current database uses sparse index compression. V1's `decode_range()` produces the correct full-length array in all cases.
...
- **No production fix is urgently needed.** The bug does not affect any current user-facing behavior.
- Share this document with the team for awareness.

### Short-term (defensive fix, ~1 hour)
- Add `expand_range(tree_node, range_array)` after `decode_range()` in V1 (line 199) and V3 (line 590) of `spot_generation.py`.
- This is a zero-risk defensive fix — `expand_range()` is a no-op when `indices` is absent.
- Write a unit test with a synthetic sparse-indices node to prove the fix works.

### Medium-term (when trainer expands to postflop)
- When the trainer is extended to support postflop streets, V1/V3 will need the full `RangeCalculator.resolve_range()` pipeline (walk-back + suit swaps), not just `expand_range()`.
- At that point, consider deprecating V1/V3 entirely and routing all traffic through V2/V4.

### Decision for team
- **Option A:** Apply the defensive `expand_range()` fix now (1 hour, zero risk). Recommended.
- **Option B:** Do nothing — document as known latent issue, revisit when trainer goes postflop.
- **Option C:** Deprecate V1/V3 now and route all FE traffic through V2/V4. Higher effort but eliminates the divergence permanently.
