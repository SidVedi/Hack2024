...
| Test | Endpoint | Spot | Variant | Research | Result |
|------|----------|------|---------|----------|--------|
| 1 | V1 `/trainer/generate-next-hand` | `unopened` | Omaha | `full_tree` | No divergence |
| 2 | V1 `/trainer/generate-next-hand` | `facingOpen` (BTN vs CO) | Omaha | `full_tree` | No divergence |
| 3 | V1 `/trainer/generate-next-hand` | `facingOpen` (BTN vs Any) | Omaha | `postflop_only` | No divergence |

### Test Parameters

```
variant=omaha, game=cash, site=NoRake, numPlayers=6, stackInBB=100,
BB=0.25, blindStructure=Regular, openRaise=3.5
```
...
**Conclusion:** All range values are 1.0 (full starting range) across all 3 tests. No sparse indices present in any test. Array is always full `NUM_COMBOS` length (270725 for Omaha). The `research` type (`full_tree` vs `postflop_only`) makes no difference at the preflop level.

### Strategy Array (breakpoint at `trainer_helper.py:364`)

| Property | Test 2: `full_tree` | Test 3: `postflop_only` | Match? |
|----------|---------------------|--------------------------|--------|
| `decoded_strategy_values.shape` | `(3, 270725)` | `(3, 270725)` | Yes |
| `decoded_strategy_values.min` | `-0.0` | `-0.0` | Yes |
| `decoded_strategy_values.max` | `1.0` | `1.0` | Yes |

3 actions = likely Fold / Call / Raise. Each row contains a probability (0.0–1.0) for each of the 270,725 Omaha hand combos.

### EV Array (breakpoint at `trainer_helper.py:364`)

| Property | Test 2: `full_tree` | Test 3: `postflop_only` | Match? |
|----------|---------------------|--------------------------|--------|
| `decoded_ev_values.shape` | `(3, 270725)` | `(3, 270725)` | Yes |
| `decoded_ev_values.min` | `-5.19` | `-5.19` | Yes |
| `decoded_ev_values.max` | `12.01` | `12.01` | Yes |

3 rows = EV (in BB) for each action for each hand combo. E.g., hand index 0: Fold EV = 0.0, Call EV = -3.24, Raise EV = -4.97 (a weak hand where folding is optimal).

**Conclusion:** Strategy and EV arrays are decoded from the same tree node bytes via `decode_strategy_ev()` in both V1 and V2. For preflop, both paths produce identical results. The `research=full_tree` and `research=postflop_only` trees produce byte-for-byte identical strategy and EV arrays at the preflop level.
